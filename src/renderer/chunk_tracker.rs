//! Tracker d'état des chunks pour éviter les doublons dans le pipeline

use crate::voxel::ChunkPos;
use std::collections::HashSet;
use std::sync::{Arc, RwLock};

/// Track l'état des chunks dans le pipeline de génération/meshing
...
impl ChunkTracker {
    ...
    pub fn mark_pending_generation(&mut self, pos: ChunkPos) -> bool {
        ...
        if self.generated.contains(&pos) {
            eprintln!("[ChunkTracker] WARNING: Chunk {:?} already generated, skipping gen request", pos);
            return false;
        }
        ...
        if self.pending_generation.contains(&pos) || self.generating.contains(&pos) {
            ...
            if self.pending_generation.contains(&pos) {
                eprintln!("[ChunkTracker] WARNING: Chunk {:?} already in pending_generation. This indicates duplicate request calls.", pos);
            }
            return false;
        }
        ...
        F: Fn(ChunkPos) -> bool + Send + Sync,
    {
        ...
        if !non_existent.is_empty() {
            eprintln!("[Cleanup] Removed {} non-existent chunks from pending_gen (remaining: {})",
            ...
            if non_existent.len() > 10 {
                eprintln!("  ... and {} more", non_existent.len() - 10);
            }
        }
    }
}
...
pub struct SharedChunkTracker(Arc<RwLock<ChunkTracker>>);

impl SharedChunkTracker {
    pub fn new() -> Self {
        Self(Arc::new(RwLock::new(ChunkTracker::new())))
    }
    ...
    pub fn cleanup_out_of_bounds(&self, max_y: i32) {
        if let Ok(mut tracker) = self.0.write() {
            ...
            if before != after {
                eprintln!("[Cleanup] Removed {} out-of-bounds chunks from pending_generation (was: {}, now: {})",
...
impl Clone for SharedChunkTracker {
    fn clone(&self) -> Self {
