...
use crate::voxel::{VoxelWorld, VoxelChunk};
use crate::voxel::chunk::{LocalVoxelId, CHUNK_VOLUME};
...
impl ChunkGenerationQueue {
                    ...
                    match rx_req.recv() {
                        Ok(req) => {
                            ...
                            if already_generated {
                                ...
                                eprintln!("[ChunkGen] WARNING: Duplicate request for already-generated chunk ({},{},{}). This should not happen if tracking is working correctly.",
                            ...
                            generator.generate_chunk(&mut chunk, req.cx, req.cy, req.cz);
                            let (voxels, palette) = chunk.extract_data();
                            let gen_duration = gen_start.elapsed();
                            println!("[ChunkGen] Generated chunk ({},{},{}) in {:.2}ms", req.cx, req.cy, req.cz, gen_duration.as_secs_f64() * 1000.0);
                            ...
                            if still_not_generated {
                                ...
                                eprintln!("[ChunkGen] WARNING: Race condition detected for chunk ({},{},{}). Another worker generated it first.",
                                    req.cx, req.cy, req.cz);
                                let _ = tx_res.send(ChunkGenResult {
                                    cx: req.cx,
                                    cy: req.cy,
    ...
    pub fn request_chunk(&self, cx: i32, cy: i32, cz: i32, priority: u32) {
        if let Err(_) = self.tx.send(ChunkGenRequest { cx, cy, cz, priority }) {
            eprintln!("[ChunkGen] ERROR: Failed to send chunk request ({},{},{})", cx, cy, cz);
        }
    }
...
impl MeshQueue {
    pub fn new(world: Arc<RwLock<VoxelWorld>>, num_workers: usize) -> Self {
        let (tx_req, rx_req) = unbounded();
        let (tx_res, rx_res) = unbounded();
                    ...
                    match rx_req.recv() {
                        Ok(req) => {
                            ...
                            let (vertices, should_send) = if let Ok(world) = world.read() {
                                if let Some(_chunk) = world.get_chunk_existing(req.cx, req.cy, req.cz) {
                                    ...
                                    match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                                        generate_chunk_mesh(_chunk, &world, req.cx, req.cy, req.cz, registry)
                                    })) {
                                        Ok(v) => (v, true),
                                        Err(_) => {
                                            (Vec::new(), false)
                            ...
                            if should_send && vertices.len() < 200000 {
                                println!("[Mesh] Generated mesh for chunk ({},{},{}) in {:.2}ms ({} vertices)",
                                ...
                                println!("[Mesh] Skipped mesh for chunk ({},{},{}) - chunk not found or panic", req.cx, req.cy, req.cz);
                            } else if vertices.len() >= 200000 {
                                println!("[Mesh] Skipped mesh for chunk ({},{},{}) - too many vertices ({})", req.cx, req.cy, req.cz, vertices.len());
                            }
                        }
                        Err(_) => {
                            break;
