            ...
            .show(&self.ctx, |ui| {
                Grid::new("debug_grid")
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label("Frame Time:");
                        ui.label(format!("{:.2}ms", perf_snapshot.frame_timing.as_ms()));
                        ui.end_row();

                        ui.label("CPU Time:");
                        ui.label(format!("{:.2}ms", perf_snapshot.cpu_timing.as_ms()));
                        ui.end_row();

                        ui.label("GPU Submit:");
                        ui.label(format!("{:.2}ms", perf_snapshot.gpu_timing.as_ms()));
                        ui.end_row();

                        // ECS systems timing
                        if !perf_snapshot.system_timings.is_empty() {
                            ui.separator();
                            ui.end_row();

                            ui.strong("ECS Systems:");
                            ui.end_row();

                            // Sort systems by time (descending)
                            let mut systems: Vec<_> = perf_snapshot.system_timings.iter().collect();
                            systems.sort_by(|a, b| b.1.avg.partial_cmp(&a.1.avg).unwrap_or(std::cmp::Ordering::Equal));

                            for (system_name, timing) in systems {
                                ui.label(format!("{}:", system_name));
                                ui.label(format!("{:.2}ms", timing.as_ms()));
                                ui.end_row();
                            }
                        }
                        ...
                        ui.label("Chunks:");
                        ui.label(format!("{}", perf_snapshot.memory.loaded_chunks));
                        ui.end_row();

                        ui.label("Meshes:");
                        ui.label(format!("{}", perf_snapshot.memory.loaded_meshes));
                        ui.end_row();

                        ui.label("Voxel Mem:");
                        ui.label(format!("{:.1} MB", perf_snapshot.memory.voxel_memory_mb));
                        ui.end_row();

                        ui.label("Mesh Mem:");
                        ui.label(format!("{:.1} MB", perf_snapshot.memory.mesh_memory_mb));
                        ui.end_row();
