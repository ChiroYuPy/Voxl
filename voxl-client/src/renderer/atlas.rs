...
pub fn generate_texture_atlas(
    ...
    for texture_name in &unique_textures {
        let path = voxl_common::paths::textures_dir().join(format!("{}.png", texture_name));
        if !path.exists() {
            warn!("Warning: Texture not found: {}", path.display());
    ...
    info!("Generated texture atlas: {}x{}, {} textures (grid: {}x{}, tex_size: {:.3})",
        atlas_size, atlas_size, texture_uvs.len(), grid_size, grid_size, 1.0 / grid_size as f32);

    // Sauvegarder l'atlas en PNG pour debug
    let debug_path = voxl_common::paths::debug_dir();
    fs::create_dir_all(&debug_path).unwrap_or_else(|e| {
        warn!("Failed to create debug directory: {}", e);
    });

    let atlas_debug_path = debug_path.join("atlas_debug.png");
    if let Err(e) = atlas_image.save(&atlas_debug_path) {
        warn!("Warning: Failed to save atlas debug image: {}", e);
    } else {
        info!("Saved atlas debug image to {}", atlas_debug_path.display());
    }
...
pub fn load_block_configs(
) -> Result<Vec<(String, voxl_common::voxel::BlockConfig)>, String> {
    let blocks_dir = voxl_common::paths::assets_dir().join("blocks");
    ...
    for entry in entries {
        ...
        let config: voxl_common::voxel::BlockConfig = ron::from_str(&content)
